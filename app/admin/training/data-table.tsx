...
import CreateTrainingForm from "./action/createForm";
import { ColumnFilter } from "./action/columnFilter";
import ExportTrainingCSVButton from "./action/exportBulk";
import ImportTrainingCSV from "./action/importBulk";
...
interface DataTableProps {
  columns: ColumnDef<Training, unknown>[];
  data: Training[];
}
...
export function DataTable({ columns, data }: DataTableProps) {
  ...
  const [searchColumn, setSearchColumn] = React.useState<"name" | "description">("name");
  const [importProgress, setImportProgress] = React.useState(0);

  const table = useReactTable<Training>({
    data,
    columns,
    ...
    <div>
      <div className="flex flex-col ">
        ...
        <div className="flex items-center justify-between">
          ...
          />
          <p>Create <span className="font-bold text-primary"> Trainings for each staff</span></p>
        </div>
        <div className="mb-3 flex items-start justify-between gap-4">
          ...
          <div className="overflow-hidden gap-4 basis-[30%] max-w-[30%] self-start">
            ...
            <div className="bg-primary overflow-hidden p-5 rounded-md border self-start">
              <span className="font-semibold text-white">Add Training</span>
              <CreateTrainingForm />
            </div>
          </div>
          ...
          <div className="overflow-hidden p-5 rounded-md border basis-[70%] max-w-[70%]">
            <div className="mb-3 flex items-center justify-between">
              <p className="font-semibold">
                Total Trainings : <span className="font-semibold text-primary text-xl">{data.length}</span>
              </p>
              <div className="flex gap-1">
                <ImportTrainingCSV setImportProgress={setImportProgress} />
                <ExportTrainingCSVButton data={data} />
