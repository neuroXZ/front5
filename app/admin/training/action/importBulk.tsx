...
export default function ImportTrainingCSV({ setImportProgress }: { setImportProgress: (value: number) => void }) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      // Helper to convert dd-mm-yyyy hh:mm to ISO
      // ISO-8601 passthrough
      // ISO-8601 passthrough (no skip, just pass string)
      ...
      for (let i = 0; i < dataLines.length; i++) {
        ...
        const [name, description, startDate, endDate] = line.split(',').map(val => val.replace(/^"|"$/g, '').trim());
        if (!name) continue;
