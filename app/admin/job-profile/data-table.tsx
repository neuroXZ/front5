...
import CreatePositionForm from "./action/createForm";
import { ColumnFilter } from "./action/columnFilter";
import ExportPositionCSVButton from "./action/exportBulk";
import ImportPositionCSV from "./action/importBulk";
...
export function DataTable({ columns, data, grades }: DataTableProps) {
  const [sorting, setSorting] = React.useState<SortingState>([]);
  const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>([]);
  const [columnVisibility, setColumnVisibility] = React.useState<VisibilityState>({});
  const [searchColumn, setSearchColumn] = React.useState<"name" | "description" | "grade">("name");
          ...
          <div className="overflow-hidden p-5 rounded-md border basis-[70%] max-w-[70%]">
            <div className="mb-3 flex items-center justify-between">
              ...
              <div className="flex gap-1">
                <ImportPositionCSV setImportProgress={setImportProgress} />
                <ExportPositionCSVButton data={data} uType={grades || []} />
