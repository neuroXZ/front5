...
import CreateGradeForm from "./action/createForm";
import { ColumnFilter } from "./action/columnFilter";
import ExportGradeCSVButton from "./action/exportBulk";
import ImportGradeCSV from "./action/importBulk";
...
interface DataTableProps {
  columns: ColumnDef<Grade, unknown>[];
  data: Grade[];
}
...
export function DataTable({ columns, data }: DataTableProps) {
  ...
  const table = useReactTable<Grade>({
    data,
    columns,
    ...
    <div>
      <div className="flex flex-col ">
        ...
        <div className="flex items-center justify-between">
          ...
          />
          <p>Create <span className="font-bold text-primary"> Grades for each staff</span></p>
        </div>
        <div className="mb-3 flex items-start justify-between gap-4">
          ...
          <div className="overflow-hidden gap-4 basis-[30%] max-w-[30%] self-start">
            ...
            <div className="bg-primary overflow-hidden p-5 rounded-md border self-start">
              <span className="font-semibold text-white">Add Grade</span>
              <CreateGradeForm />
            </div>
          </div>
          ...
          <div className="overflow-hidden p-5 rounded-md border basis-[70%] max-w-[70%]">
            <div className="mb-3 flex items-center justify-between">
              <p className="font-semibold">
                Total Grades : <span className="font-semibold text-primary text-xl">{data.length}</span>
              </p>
              <div className="flex gap-1">
                <ImportGradeCSV setImportProgress={setImportProgress} />
                <ExportGradeCSVButton data={data} />
