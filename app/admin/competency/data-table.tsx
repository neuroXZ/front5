...
import CreateCompetencyForm from "./action/createForm";
import { ColumnFilter } from "./action/columnFilter";
import ExportCompetencyCSVButton from "./action/exportBulk";
import ImportCompetencyCSV from "./action/importBulk";
import DeleteBulk from "./action/deleteBulk";
import { DatabaseNav } from "@/components/custom/smallNav/database-nav";
import { Progress } from "@/components/ui/progress"


interface DataTableProps {
  columns: ColumnDef<Competency, unknown>[];
  data: Competency[];
  clusters?: { id: string; name: string }[];
}

export function DataTable({ columns, data, clusters }: DataTableProps) {
  const [sorting, setSorting] = React.useState<SortingState>([]);
  const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>([]);
  const [columnVisibility, setColumnVisibility] = React.useState<VisibilityState>({});
  const [searchColumn, setSearchColumn] = React.useState<"name" | "description" | "cluster">("name");
  const [importProgress, setImportProgress] = React.useState(0);

  const table = useReactTable<Competency>({
    data,
    columns,
        ...
        <div className="mb-3 flex items-start justify-between gap-4">
          ...
          <div className="overflow-hidden gap-4 basis-[30%] max-w-[30%] self-start">
            ...
            <div className="bg-primary overflow-hidden p-5 rounded-md border self-start">
              <span className="font-semibold text-white">Add Competency</span>
          ...
          <div className="overflow-hidden p-5 rounded-md border basis-[70%] max-w-[70%]">
            <div className="mb-3 flex items-center justify-between">
              <p className="font-semibold">
                Total Competency : <span className="font-semibold text-primary text-xl">{data.length}</span>
              </p>
              <div className="flex gap-1">
                <ImportCompetencyCSV setImportProgress={setImportProgress} />
                <ExportCompetencyCSVButton data={data} uType={clusters || []} />
