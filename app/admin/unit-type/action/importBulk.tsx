...
export default function ImportUnitTypeCSV({ setImportProgress }: { setImportProgress: (value: number) => void }) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 0; i < dataLines.length; i++) {
        ...
        const [name, description] = line.split(',').map(val => val.replace(/^"|"$/g, '').trim());
        if (!name) continue;
